  ...
  "panels": [
    {
      ...
      "description": "ilo_power_current_watt / ilo_power_average_watt / ilo_power_min_watt / ilo_power_max_watt, by host (Attributes['instance'] is the ilo IP set by collector-and-ilo.yaml's relabel_configs). No existing Grafana source dashboard for iLO was checked in - authored directly from the ilo_exporter's published metric names (MauveSoftware/ilo_exporter).",
      ...
      "targets": [
        {
          ...
          "datasource": { "type": "grafana-clickhouse-datasource", "uid": "${ch_uid}" },
          "format": "time_series",
          "rawSql": "SELECT toStartOfInterval(TimeUnix, INTERVAL 1 MINUTE) AS time, concat(Attributes['instance'], ' ', Attributes['name']) AS metric, avg(Value) AS value FROM otel.otel_metrics_gauge WHERE MetricName = 'ilo_chassis_temperature_current' AND $__timeFilter(TimeUnix) GROUP BY time, metric ORDER BY time"
        }
      ]
    },
    {
      ...
      "targets": [
        {
          ...
          "datasource": { "type": "grafana-clickhouse-datasource", "uid": "${ch_uid}" },
          "format": "time_series",
          "rawSql": "SELECT toStartOfInterval(TimeUnix, INTERVAL 1 MINUTE) AS time, concat(Attributes['instance'], ' ', Attributes['name']) AS metric, avg(Value) AS value FROM otel.otel_metrics_gauge WHERE MetricName = 'ilo_chassis_fan_current_percent' AND $__timeFilter(TimeUnix) GROUP BY time, metric ORDER BY time"
        }
      ]
    },
    {
